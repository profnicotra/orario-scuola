# hybrid_timetable_postgres.py placeholder will be overwritten below
# hybrid_timetable_postgres.py (vincoli estesi + capacità per periodi)
...
class TimetableSolver:
    def __init__(self, classes, subjects, teachers, teacher_subjects, availability,
                 ...
                 class_day_capacity, class_date_capacity_periods,
                 default_day_capacity=6):
        ...
        self.class_date_capacity_periods = class_date_capacity_periods
        ...
        self.class_hours_count={}
        self.initial_zero_domain=[]

    def _daily_cap_for(self, cid: str, d: date) -> int:
        caps = [mh for (sd,ed,mh) in self.class_date_capacity_periods.get(cid, []) if sd <= d <= ed]
        if caps:
            return min(caps)
        return self.class_day_capacity.get((cid, d.weekday()), self.default_day_capacity)
    ...
    def candidate_assignments(self, L: Lesson):
        ...
        for d in self.school_calendar.school_days:
            ...
            if any(self._daily_cap_for(cid, d) <= 0 for cid in members): continue
...
def load_config_from_db(conn, start_date:date, days:int|None, weeks:int|None):
    ...
    cur.execute('SELECT class_id, start_date, end_date, max_hours FROM class_date_capacity_periods')
    datecap={}
    for cid,sd,ed,mh in cur.fetchall():
        datecap.setdefault(cid, []).append((sd,ed,int(mh)))
    school_days=[]
    ...
    return (classes,subjects,teachers,teacher_subjects,merge_groups,availability,SchoolCalendar(school_days),
            teacher_blackouts,class_stage,class_start_dates,daycap,datecap)
...
def main():
    ap=argparse.ArgumentParser(description='Hybrid School Timetable (vincoli estesi + capacità per periodi)')
    ...
    with connect_pg(args.dsn) as conn:
        (classes,subjects,teachers,teacher_subjects,merge_groups,availability,school_calendar,
         teacher_blackouts,class_stage,class_start_dates,daycap,datecap)=load_config_from_db(conn,args.start,args.days,args.weeks)
        solver=TimetableSolver(classes,subjects,teachers,teacher_subjects,availability,
                               ...
                               teacher_blackouts,class_stage,class_start_dates,daycap,datecap,default_day_capacity=6)
